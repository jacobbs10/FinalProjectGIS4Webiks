...
const Location = require('../models/EmergencyLocModel');
...
router.post("/cat", authMiddleware, async (req, res) => {
    try {
        ...
        const query = {
            ...
            ...(sub_categories && Array.isArray(sub_categories) && sub_categories.length > 0
                ? { 'properties.sub_category': { $in: sub_categories } }
                : {}),
...
router.post("/range", authMiddleware, async (req, res) => {
    try {
        const { coordinates, range, categories, sub_categories } = req.body;
        ...
        // Send the response with matching locations and their distances
        res.json({
            locations: locations,
            total: locations.length // Include the total count of matching locations
        });
...
router.post("/closest", authMiddleware, async (req, res) => {
    try {
        const { coordinates, categories, sub_categories, vType } = req.body;

        // Validate that coordinates is a valid [longitude, latitude] array
        if (!Array.isArray(coordinates) || coordinates.length !== 2 || typeof coordinates[0] !== 'number' || typeof coordinates[1] !== 'number') {
            return res.status(400).json({
        ...
        const query = {
            ...
            ...(sub_categories && Array.isArray(sub_categories) && sub_categories.length > 0
                ? { 'properties.sub_category': { $in: sub_categories } }
                : {}),
            ...(vType && Array.isArray(vType) && vType.length > 0
                ? { 'properties.vehicles.type': { $in: vType } } // Match at least one vehicle type
                : {}),
            ...(req.user && (req.user.role === 'Confidential' || req.user.role === 'Admin')
                ? {} // Fetch all records for Confidential/Admin users
                : { 'properties.restricted': false }) // Fetch only non-restricted records for other users
        ...
        const pipeline = [
            {
                $geoNear: {
                    ...
                    distanceField: "distance", // Field to store the calculated distance
                    spherical: true, // Use spherical geometry for distance calculation
                    query: query, // Apply the query to filter locations
                }
            },
            {
                $match: {
                    // Ensure available_personal is >= min_qty of at least one vehicle
                    $expr: {
                        $gte: [
                            "$properties.available_personal",
                            {
                                $min: "$properties.vehicles.min_qty"
                            }
                        ]
                    }
                }
            ...
            {
                $limit: 1 // Fetch only the nearest location
            }
        ];
        ...
        // Send the response with the nearest location
        res.json({
            location: nearestLocation[0], // Return the nearest location
            distance: nearestLocation[0].distance // Include the distance in the response
        });
    } catch (error) {
        console.error("Error fetching the closest location:", error);
        res.status(500).json({ message: "Server error" });
    }
});
...
router.post("/location", authMiddleware, isAdmin, async (req, res) => {
    try {
      ...
      const {
        ...
        email, phone, site, loc_status, photo, coordinates,
        equipment, vehicles, total_personal, available_personal, min_personal
    } = req.body;
      
      // Validate required fields
      if (!category || !sub_category || !loc_name || !address || !description || !coordinates) {
        return res.status(400).json({ 
          success: false, 
          message: 'Missing required fields: category, sub_category, loc_name, address, description, and coordinates are required'
        });
      }
  ...
  router.post("/bulk", authMiddleware, isAdmin, async (req, res) => {
    try {
        ...
        for (const n of locations) {
            ...
            if (!category || !sub_category || !loc_name || !address || !description || !Array.isArray(coordinates) || coordinates.length !== 2) {
            ...
            toInsert.push({
                ...
                properties: {
                    id, category, sub_category, loc_name, address, description, restricted,
                    email, phone, site, loc_status, photo, equipment, vehicles,
    ...
    try {        
        const updateData = {
            $set: {
                ...
                'properties.sub_category': sub_category,
                ...
                'properties.available_personal': available_personal,
                'properties.min_personal': min_personal,
